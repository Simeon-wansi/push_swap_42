...
void improved_radix_sort(int *stack_a, int *stack_b, int *len_a, int *len_b)
{
    int min;
    int i;
    int max_bits;
    ...
    for (bit_position = 0; bit_position < max_bits; bit_position += 2)
    {
        size = *len_a;
        for (i = 0; i < size; i++)
        {
            int bits = (stack_a[0] >> bit_position) & 0x3; // Extract 2 bits
            if (bits == 0 || bits == 1)
                ra(stack_a, *len_a); // Rotate if bits are 0 or 1
            else
                pb(stack_a, len_a, stack_b, len_b); // Push to stack_b if bits are 2 or 3
        }
...
int main(int ac, char **av)
{
    ...
    improved_radix_sort(stack_a, stack_b, &len_a, &len_b);
