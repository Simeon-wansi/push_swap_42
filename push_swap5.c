...
void insertion_sort_with_operations(int *stack_a, int *stack_b, int *len_a, int *len_b)
{
    while (*len_a > 0)
    {
        ...
        // Rotate stack_b to find the correct position for the new element
        while (*len_b > 1 && stack_b[0] > stack_b[1])
        {
            rb(stack_b, *len_b);
        }
    }
...
void hybrid_sort(int *stack_a, int *stack_b, int *len_a, int *len_b)
{
    int chunk_size = 20; // Adjust based on input size
    int i;

    // Divide stack_a into chunks and push to stack_b
    while (*len_a > 0)
    {
        for (i = 0; i < chunk_size && *len_a > 0; i++)
        {
            pb(stack_a, len_a, stack_b, len_b);
        }

        // Sort the current chunk in stack_b using Insertion Sort
        insertion_sort_with_operations(stack_a, stack_b, len_a, len_b);
    }
...
int main(int ac, char **av)
{
    ...
    hybrid_sort(stack_a, stack_b, &len_a, &len_b);
